...
box_binary<-function(w, p11, p12, in1, in2){
  w*rnorm(10000,p11,sqrt(p11*(1-p11)/in1))+(1-w)*rnorm(10000,p12,sqrt(p12*(1-p12)/in2))
}
...
En3_binary <-  function(RRgo, n2, alpha, beta, p0, w, p11, p12, in1, in2){
...
EPsProg_binary <-  function(RRgo, n2, alpha, beta, step1, step2, p0, w, p11, p12, in1, in2, gamma){
...
# Utility function
utility_binary <-  function(n2){

  pg    <-  Epgo_binary(RRgo = RRgo, n2 = n2, p0 = p0, w = w, p11 = p11, p12 = p12, in1 = in1, in2 = in2)
  ...
  if(K2+K3>K){
    ...
    prob1 <-  EPsProg_binary(RRgo = RRgo, n2 = n2, alpha = alpha, beta = beta,
                             ...
                             p0 = p0, w = w, p11 = p11, p12 = p12, in1 = in1, in2 = in2, gamma = gamma)
...
n3_skipII_binary <-function(alpha, beta, p0, median_prior){
  ...
  ceiling(((2*(qnorm(1-alpha)*sqrt(2*(1-((p0 + median_prior)/2))/((p0 + median_prior)/2))+qnorm(1-beta)*sqrt((1-p0)/p0+(1-median_prior)/median_prior))^2)/median_RR^2))

}

# expected probability of a successful program based on median_prior
EPsProg_skipII_binary <-function(alpha, beta, step1, step2, p0, median_prior, w, p11, p12, in1, in2, gamma){
