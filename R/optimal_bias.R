...
optimal_bias <- function(w,  hr1, hr2, id1, id2,
  ...
  if(skipII==TRUE){
    ...
    res <- utility_skipII_bias(alpha = alpha, beta = beta, xi3 = xi3,
                              c03 = c03, c3 = c3,
                              b1 = b1, b2 = b2, b3 = b3,
  ...
  LAMBDA  <- seq(lambdamin, lambdamax, steplambda)
  ALPHACI <- seq(alphaCImin, alphaCImax, stepalphaCI)
  
  cat("Optimization progress:", fill = TRUE)
  cat("", fill = TRUE)
  pb <- txtProgressBar(min = 0, max = length(LAMBDA), style = 3, label = "Optimization progess")
  ...
  for(l in 1:length(LAMBDA)){
    ...
    for(j in 1:length(HRGO)){
      ...
      clusterExport(cl, c("pmvnorm", "dmvnorm", "prior_tte", "Epgo_bias", "Ed3_bias",
                          "EPsProg_bias", "alpha", "beta",
                          "steps1", "steps2", "stepm1", "stepm2", "stepl1", "stepl2",
                          "K", "N", "S", "gamma", "fixed",
                          "xi2", "xi3", "c2", "c3", "c02", "c03",
                          "b1", "b2", "b3", "w", "HRgo", "lambda",
                          "hr1", "hr2", "id1", "id2"), envir = environment())
      
      
      result <- parSapply(cl, D2, utility_bias, HRgo, lambda, w, hr1, hr2, id1, id2,
                          alpha, beta, xi2, xi3,
                          c2, c3, c02, c03, 
                          K, N, S,
    ...
    if(!fixed){
      ...
      calresult <-  data.frame(u = round(Eud,2), HRgo = HRGO[J], d2 = D2[I], d3 = d3, d = D2[I] + d3,
                            n2 = n2, n3 = n3, n = n2 + n3,
  ...
  index   <- which(calresult$Eud == max(calresult$Eud))
  
  result  <- calresult[index,]
