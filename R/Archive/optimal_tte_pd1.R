...
#'
#' @export
optimal_tte_pd <- function(w,  hr1, hr2, id1, id2,
                        d2min, d2max, stepd2,
  ...
  for(j in 1:length(HRGO)){
    ...
    clusterExport(cl, c("pmvnorm", "dmvnorm", "prior_tte", 
                        "Epgo_tte", "Ed3_tte", "Ed3_tte_pdc",
                        "EPsProg_tte", "EPsProg_tte_pdb", "alpha", "beta",
    ...
    if(pd==1){
      ...
      result <- parSapply(cl, D2, utility_tte_pdc, 
                          HRgo, w, hr1, hr2, id1, id2,
                          alpha, beta, xi2, xi3,
                          c2, c3, c02, c03, 
                          K, N, S,
  ...
  if(!fixed){
    result <-  data.frame(u = round(Eud,2), 
                ...
                b1 = b1, b2 = b2, b3 = b3, pd = pd, gamma = gamma)
  }else{
    result <-  data.frame(u = round(Eud,2), 
                HRgo = HRGO[J], d2 = D2[I], 
                d3 = d3, d = D2[I] + d3,
