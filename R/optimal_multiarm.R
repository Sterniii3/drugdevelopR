...
optimal_multiarm <- function(hr1, hr2, ec,
  ...
  for(j in 1:length(HRGO)){
    ...
    clusterExport(cl, c("pmvnorm", "dmvnorm", "pgo", "ss", "Ess",
                        "PsProg", "alpha", "beta",
    ...
    result <- parSapply(cl, N2, utility_multiarm, HRgo,
                        alpha,beta,hr1,hr2,
  ...
  result <-  data.frame(u = round(Eud,2), HRgo = HRGO[J], n2 = N2[I], n3 = n3, n = n2 + n3,
