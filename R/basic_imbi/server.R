...
# Utility function
utilityf_time <-  function(HRgo,d2,alpha,beta,theta,
                           p2,p3,c2,c02,c3,c03,b1,b2,b3,K,
                           steps1,steps2,stepm1,
                           stepm2,stepl1,stepl2){

   pg    <-  pgof_time(HRgo = HRgo, d2 = d2, 
                       theta = theta)
   d3    <-  Ed3f_time(HRgo = HRgo, d2 = d2, 
   ...
   if(K2+K3>K){
      ...
      prob1 <-  EPsProgf_time(HRgo = HRgo, d2 = d2, 
                  alpha = alpha, beta = beta, theta = theta,
                  step1 = steps1, step2 =  steps2)
      prob2 <-  EPsProgf_time(HRgo = HRgo, d2 = d2, 
                  alpha = alpha, beta = beta, theta = theta,
                  step1 =  stepm1, step2 =  stepm2)
      prob3 <-  EPsProgf_time(HRgo = HRgo, d2 = d2, 
                  alpha = alpha, beta = beta, theta = theta,
      ...
      return(c(EU, d2, d3, SP, pg, K2, K3, 
               prob1, prob2, prob3, n2, n3))
   }
}
...
EPsProgf_binary <-  function(RRgo, n2, alpha, beta, 
                             p0, p1, step1, step2){
   # Phase II
   rho   = -log(p1/p0)
   Var   = (2/n2)*((1-p0)/p0+(1-p1)/p1)
...
# Utility function
utilityf_binary <-  function(RRgo,n2,alpha,beta,p0,p1,
                             c2,c02,c3,c03,b1,b2,b3,K,
                             steps1,steps2,stepm1,
                             stepm2,stepl1,stepl2){

   pg    <-  pgof_binary(RRgo = RRgo, n2 = n2, 
                         p0 = p0, p1 = p1)
   n3    <-  En3f_binary(RRgo = RRgo, n2 = n2, 
                         alpha = alpha, beta = beta, 
                         p0 = p0, p1 = p1)
   ...
   if(K2+K3>K){
      ...
      prob1 <-  EPsProgf_binary(RRgo = RRgo, n2 = n2, 
                     alpha = alpha, beta = beta, 
                     p0 = p0, p1 = p1,
                     step1 = steps1, step2 =  steps2)
      prob2 <-  EPsProgf_binary(RRgo = RRgo, n2 = n2, 
                     alpha = alpha, beta = beta, 
                     p0 = p0, p1 = p1,
                     step1 =  stepm1, step2 =  stepm2)
      prob3 <-  EPsProgf_binary(RRgo = RRgo, n2 = n2, 
                     alpha = alpha, beta = beta, 
                     p0 = p0, p1 = p1,
                     step1 =  stepl1, step2 = stepl2)
...
# Utility function
utilityf_normal <-  function(kappa,n2,alpha,beta,Delta,
                             c2,c02,c3,c03,b1,b2,b3,K,
                             steps1,steps2,stepm1,
                             stepm2,stepl1,stepl2){

   pg    <-  pgof_normal(kappa = kappa, n2 = n2, 
                         Delta = Delta)
   n3    <-  En3f_normal(kappa = kappa, n2 = n2, 
                         alpha = alpha, beta = beta, 
                         Delta = Delta)
   ...
   if(K2+K3>K){
      ...
      prob1 <-  EPsProgf_normal(kappa = kappa, n2 = n2, 
                  alpha = alpha, beta = beta, Delta = Delta,
                  step1 = steps1, step2 =  steps2)
      prob2 <-  EPsProgf_normal(kappa = kappa, n2 = n2, 
                  alpha = alpha, beta = beta, Delta = Delta,
                  step1 =  stepm1, step2 =  stepm2)
      prob3 <-  EPsProgf_normal(kappa = kappa, n2 = n2, 
                  alpha = alpha, beta = beta, Delta = Delta,
                  step1 =  stepl1, step2 = stepl2)
...
output$table <- renderTable({
  ...
  Select = input$Select
   ...
   if(Select==1){
     ...
     stepl2 <- 0
     y = function(x){
        result = utilityf_time(tres,x,alpha,beta,theta,
                               p2,p3,c2,c02,c3,c03,b1,b2,b3,
                               K,steps1,steps2,stepm1,
                               stepm2,stepl1,stepl2)
        return(result)
     }
  }
   if(Select==2){
      ...
      stepl2 <- 0
      y = function(x){
         result = utilityf_binary(tres,x,alpha,beta,
                                  p0,p1,c2,c02,c3,c03,b1,b2,b3,
                                  K,steps1,steps2,stepm1,
                                  stepm2,stepl1,stepl2)
         return(result)
      }
   ...
   if(Select==3){
     ...
     stepl2 <- Inf
     y = function(x){
        result = utilityf_normal(tres,x,alpha,beta,
                                 Delta,c2,c02,c3,c03,b1,b2,b3,
                                 K,steps1,steps2,stepm1,
                                 stepm2,stepl1,stepl2)
        return(result)
     }
   ...
   ufkt <- numfkt <- tresfkt <- d3fkt<- spfkt <- pgofkt <- 
      sp1fkt <-sp2fkt <- sp3fkt <- dfkt <- K2fkt <- 
      K3fkt <- n2fkt<- n3fkt<- 
      matrix(0,length(NUM),length(TRES))
   ...
   if(refresh==0){
      load(file=paste0(mainPath, 
                       "optimizationresults.RData"))
      }else{DF = NULL}
   if(Select==1){
      DF <- rbind(DF,data.frame(u=Eud,HR=HR,
              HRgo=TRES[J],d2=format(num2tot,digits=0),
              d3=format(num3tot,digits=0),
              d=format(numtot,digits=0),
              n2=format(n2tot,digits=0),
              n3=format(n3tot,digits=0),
              n=format(n2tot+n3tot,digits=0),
              K=format(K,digits=0),
              pgo=pg,sProg=prob,
              sProg1=prob1,sProg2=prob2,sProg3=prob3,
              K2=k2,K3=k3, xi2=p2,xi3=p3,
   ...
   if(Select==2){
      DF <- rbind(DF,data.frame(u=Eud,P0=p0,P1=p1,RR=p1/p0,
              RRgo=TRES[J],
              n2=format(num2tot,digits=0),
              n3=format(num3tot,digits=0),
