...
The drugdevelopR package enables us to otimally plan such programs using a utility maximizing approach. Specifically, it calculates
...
Within our drug development program, we will compare our experimental treatment *exper* to the control treatment *contro*. Building on available data, we expect that the mean tumor difference amounts to $20±1 \mathrm{mm}$ for *contro* and $15±1 \mathrm{mm}$ for *exper*. Furhtermore, the variance is assumed to be 8. The treatment effect is given as standardized differnce in mean ($\Delta=\frac{\mu_{contro} - \mu_{exper}}{\sigma}$). Thus, we obtain a standardized treatment difference of $\Delta_1 = 0.625$. 
...
* `c02` and `c03` are fixed costs for phase II and phase III respectively. We will set the phase II costs to 15 and the phase III costs to 20 (in $10^5$€), i.e we have fixed costs of 1 500 000€ in phase II and 2 000 000€ in phase III.  
* `c2` and `c3` are the costs in phase II and phase III per patient. We will set them to be 0.675 in phase II and 0.72 in phase III. Again this values are given in $10^5$€, i.e we have per patient costs of 67 500€ in phase II and 72 000€ in phase III.
* `b1`, `b2` and `b3` are the expected small, medium and large benefit categories for successfully launching the treatment on the market for each effect size category in $10^5$€. We will define a small benefit of 3000, a medium benefit of 8000, and a large benefit of 10000. The effect size categories directly correspond to the treatment effect, i.e. if the the treatment effect is between 0 and 0.5 (in standardized differences of mean) we have a small treatment effect, hence yielding expected benefits of the drug development program of 300 000 000€.
...
```{r,eval = FALSE}
 ...
 res <- optimal_normal(w=0.3, Delta1 = 0.625, Delta2 = 0.325, 
                       ...
                       b1 = 3000, b2 = 8000, b3 = 10000, fixed=TRUE) 
```
...
Optimization result:
        ...
        u Kappa n2  n3   n pgo sProg Delta   K K2  K3 sProg1 sProg2 sProg3 steps1
1 3259.83  0.06 84 166 250   1  0.85 0.625 Inf 72 139   0.68   0.17   0.01      0
  stepm1 stepl1 alpha beta c02 c03    c2   c3   b1   b2    b3 gamma
1    0.5    0.8  0.05  0.1  15  20 0.675 0.72 3000 8000 10000     0
```


The program returns a total of thirteen values and the input values. For now, we will only look at the most important ones:

* `res$n2` is the optimal sample size for phase II and `res$n3` the resulting sample size for phase III. We see that the optimal scenario requires 84 participants in phase II and 166 participants in phase III. 
...
* `res$u` is the expected utility of the program for the optimal sample size and threshold value. In our case it amounts to 3259.83, i.e we have an expected utility of 325 983 000€. 
