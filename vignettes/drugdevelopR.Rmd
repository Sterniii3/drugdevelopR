...
Suppose we are planning a drug development program testing the superiority of an experimental treatment over a control treatment. Our program consists of an exploratory phase II trial which is, in case of promising results, followed by a confirmatory phase III trial.
...
* `c02` and `c03` are the maximal costs in phase II and phase III per patient. We will set them to be 15 in phase II and 20 in phase III. (TODO: Stimmt das? Was ist die Interpretation hiervon von diesen Kosten? Euro?)
...
(TODO: Das müsste noch überarbeitet werden. Hier auch nur die drei Basics erklären.)

After setting all these input parameters we now take a closer look at the output of the program. The program returns a total of 13 values, which will now be explained in a bit more detail. The most important returns are the maximal expected utility of the program `u` and the optimal sample sizes for phase II and III `n2` and `n3`, as well as the optimal threshold value to go to phase III `kappa`.
...
This tutorial only covered drugdevelopR's basic setting. Luckily, its functionality extends to a multitude of different settings. If the example above doesn't suit your needs, you can adapt it to your specific setting:
