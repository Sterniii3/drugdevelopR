...
```{r}
in_hours <- function(x){
  x <- as.numeric(x, units = "hours")
  return(x)
}

data.frame(`Basic 01` = signif(dur01, digits = 3),
           `Bias 02` = signif(dur02, digits = 3),
           `Multitrial 03` = signif(dur03, digits = 3),
           `Multiarm 04` = signif(dur04, digits = 3),
           `Multiple 05` = signif(dur05, digits = 3),
           `Total` = paste(signif(in_hours(dur01 + dur02 + dur03 + dur04 + 
                                             dur05), digits = 3), "hours")) %>% 
